<?php  
	...
	{
		/* Lấy sản phẩm detail */
		$row_detail = $d->rawQueryOne("select type, id, ten$lang, tenkhongdauvi, tenkhongdauen, mota$lang, tinhnang$lang,noidung$lang, masp, luotxem, id_brand, id_mau, id_size, id_list, id_cat, id_item, id_sub, id_tags, photo, options, giacu,giakm, giamoi,mauxe, gia,banner,noibat$lang,thongso$lang,tongquan$lang from #_product where id = ? and type = ? and hienthi > 0 limit 0,1",array($id,$type));
		...
		$sql = "select photo, ten$lang, tenkhongdauvi, tenkhongdauen,banner,giamoi, gia, giakm,giacu, id,motangan$lang from #_product where $where order by stt,id desc $limit";
	...
	else if($idl!='')
	{
		...
		$startpoint = ($curPage * $per_page) - $per_page;
		$limit = " limit ".$startpoint.",".$per_page;
		$sql = "select photo, ten$lang, tenkhongdauvi, tenkhongdauen,banner,giamoi, gia, giakm,giacu, id from #_product where $where order by stt,id desc $limit";
		$product = $d->rawQuery($sql,$params);
		$sqlNum = "select count(*) as 'num' from #_product where $where order by stt,id desc";
		$count = $d->rawQueryOne($sqlNum,$params);
		$total = $count['num'];
	...
	else
	{
		...
		$startpoint = ($curPage * $per_page) - $per_page;
		$limit = " limit ".$startpoint.",".$per_page;
		$sql = "select photo, ten$lang, tenkhongdauvi, tenkhongdauen,banner,giamoi, gia, giakm,giacu,mauxe,motangan$lang, id from #_product where $where order by stt,id desc $limit";
		$product = $d->rawQuery($sql,$params);
		$sqlNum = "select count(*) as 'num' from #_product where $where order by stt,id desc";
		$count = $d->rawQueryOne($sqlNum,$params);
		$total = $count['num'];
