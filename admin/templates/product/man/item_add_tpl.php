    ...
    <form class="validation-form" novalidate method="post" action="<?=$linkSave?>" enctype="multipart/form-data">
        ...
        <div class="row">
		                        ...
		                        <div class="tab-content" id="custom-tabs-three-tabContent-lang">
		                            <?php foreach($config['website']['lang'] as $k => $v) { ?>
		                                <div class="tab-pane fade show <?=($k=='vi')?'active':''?>" id="tabs-lang-<?=$k?>" role="tabpanel" aria-labelledby="tabs-lang">
		                                    ...
		                                    <?php if(isset($config['product'][$type]['mota_ngan']) && $config['product'][$type]['mota_ngan'] == true) { ?>
		                                        <div class="form-group">
		                                            <label for="motangan<?=$k?>">Mô tả ngắn(<?=$k?>):</label>
		                                            <textarea class="form-control" name="data[motangan<?=$k?>]" id="motangan<?=$k?>" rows="5" placeholder="Mô tả (<?=$k?>)"><?=htmlspecialchars_decode(@$item['motangan'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>
		                                    ...
		                                    <?php } ?>
		                                    <?php if(isset($config['product'][$type]['tinhnang']) && $config['product'][$type]['tinhnang'] == true) { ?>
		                                        <div class="form-group">
		                                            <label for="tinhnang<?=$k?>">Tính năng (<?=$k?>):</label>
		                                            <textarea class="form-control for-seo <?=(isset($config['product'][$type]['tinhnang_cke']) && $config['product'][$type]['tinhnang_cke'] == true)?'form-control-ckeditor':''?>" name="data[tinhnang<?=$k?>]" id="tinhnang<?=$k?>" rows="5" placeholder="Tính năng (<?=$k?>)"><?=htmlspecialchars_decode(@$item['tinhnang'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>
		                                    <?php if(isset($config['product'][$type]['noibat']) && $config['product'][$type]['noibat'] == true) { ?>
		                                        <div class="form-group">
		                                            <label for="noibat<?=$k?>">Nổi bật (<?=$k?>):</label>
		                                            <textarea class="form-control <?=(isset($config['product'][$type]['noibat_cke']) && $config['product'][$type]['noibat_cke'] == true)?'form-control-ckeditor':''?>" name="data[noibat<?=$k?>]" id="noibat<?=$k?>" rows="5" placeholder="Nội dung (<?=$k?>)"><?=htmlspecialchars_decode(@$item['noibat'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>
		                                    <?php if(isset($config['product'][$type]['tongquan']) && $config['product'][$type]['tongquan'] == true) { ?>
		                                        <div class="form-group">
		                                            <label for="tongquan<?=$k?>">Tổng quan (<?=$k?>):</label>
		                                            <textarea class="form-control <?=(isset($config['product'][$type]['tongquan_cke']) && $config['product'][$type]['tongquan_cke'] == true)?'form-control-ckeditor':''?>" name="data[tongquan<?=$k?>]" id="tongquan<?=$k?>" rows="5" placeholder="Nội dung (<?=$k?>)"><?=htmlspecialchars_decode(@$item['tongquan'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>
		                                    <?php if(isset($config['product'][$type]['tongquan']) && $config['product'][$type]['thongso'] == true) { ?>
		                                        <div class="form-group">
		                                            <label for="thongso<?=$k?>">Thông số (<?=$k?>):</label>
		                                            <textarea class="form-control <?=(isset($config['product'][$type]['thongso_cke']) && $config['product'][$type]['thongso_cke'] == true)?'form-control-ckeditor':''?>" name="data[thongso<?=$k?>]" id="thongso<?=$k?>" rows="5" placeholder="Nội dung (<?=$k?>)"><?=htmlspecialchars_decode(@$item['thongso'.$k])?></textarea>
		                                        </div>
		                                    <?php } ?>
        	...
        	<div class="<?=$colRight?>">
		        ...
		        <?php } ?>
		        <?php if(isset($config['product'][$type]['banner']) && $config['product'][$type]['banner'] == true) { ?>
                    <div class="card card-primary card-outline text-sm">
                        <div class="card-header">
                        ...
                        <div class="card-body">
                            <?php
                                $photoDetail2 = UPLOAD_PRODUCT.@$item['banner'];
                                $dimension2 = "Width: 1200px - Height: 675 (".$config['product'][$type]['img_type'].")";
                                include TEMPLATE.LAYOUT."image-banner.php";
                            ?>
                        </div>
                    </div>
                <?php } ?>
	    ...
	    <div class="card card-primary card-outline text-sm">
            ...
            <div class="card-body">
                ...
                <div class="row">
				    ...
				    <?php } ?>
				    <?php if(isset($config['product'][$type]['phibienso']) && $config['product'][$type]['phibienso'] == true) { ?>
				    	<div class="form-group col-md-4">
	                        <label class="d-block" for="phibienso">Phí biển số:</label>
	                        <div class="input-group">
	                        	<input type="text" class="form-control format-price phibienso" name="data[phibienso]" id="phibienso" placeholder="Phí biển số" value="<?=@$item['phibienso']?>">
	                        	<div class="input-group-append">
	                        		<div class="input-group-text"><strong>VNĐ</strong></div>
	                        	</div>
	                        </div>
				    ...
				    <?php } ?>
				    <?php if(isset($config['product'][$type]['phisudungduongbo']) && $config['product'][$type]['phisudungduongbo'] == true) { ?>
				    	<div class="form-group col-md-4">
	                        <label class="d-block" for="phisudungduongbo">Phí sử dụng đường bộ:</label>
	                        <div class="input-group">
	                        	<input type="text" class="form-control format-price phisudungduongbo" name="data[phisudungduongbo]" id="phisudungduongbo" placeholder="Phí sử dụng đường bộ" value="<?=@$item['phisudungduongbo']?>">
	                        	<div class="input-group-append">
	                        		<div class="input-group-text"><strong>VNĐ</strong></div>
	                        	</div>
	                        </div>
				    ...
				    <?php } ?>
				    <?php if(isset($config['product'][$type]['phikiemdinh']) && $config['product'][$type]['phikiemdinh'] == true) { ?>
				    	<div class="form-group col-md-4">
	                        <label class="d-block" for="phikiemdinh">Phí kiểm định:</label>
	                        <div class="input-group">
	                        	<input type="text" class="form-control format-price phikiemdinh" name="data[phikiemdinh]" id="phikiemdinh" placeholder="Phí kiểm định" value="<?=@$item['phikiemdinh']?>">
	                        	<div class="input-group-append">
	                        		<div class="input-group-text"><strong>VNĐ</strong></div>
	                        	</div>
	                        </div>
				    ...
				    <?php } ?>
				    <?php if(isset($config['product'][$type]['baohiemtnds']) && $config['product'][$type]['baohiemtnds'] == true) { ?>
				    	<div class="form-group col-md-4">
	                        <label class="d-block" for="baohiemtnds">Bảo hiểm TNDS:</label>
	                        <div class="input-group">
	                        	<input type="text" class="form-control format-price baohiemtnds" name="data[baohiemtnds]" id="baohiemtnds" placeholder="Bảo hiểm TNDS" value="<?=@$item['baohiemtnds']?>">
	                        	<div class="input-group-append">
	                        		<div class="input-group-text"><strong>VNĐ</strong></div>
	                        	</div>
	                        </div>
			    	...
			    	<div class="form-group col-md-12"> 
				    	<label class="control-label">Mẫu xe 
				    		<button type="button" class="ml-1 btn-plus btn btn-sm btn-info">
				    			<i class="fa fa-plus"></i>
				    		</button>
				    	</label>
				    	<div id="mauxe-in" class="row">
				    		<?php 
				    		if(isset($listmauxe) && count($listmauxe)){
				    		foreach($listmauxe as $v){ ?>
				    		<div class="form-group col-md-12">
                                <div class="row">
                                    <div class="col-lg-4">
                                        <input type="text" value="<?=$v['mauxe']?>" name="mauxe[]" placeholder="Mẫu xe" class="form-control"></div>
                                    <div class="col-lg-3">
                                        <input type="text" value="<?=$v['giabanle']?>"  name="giabanle[]" placeholder="Giá bán" class="format-price form-control">
                                    </div>
                                    <div class="col-lg-1">
		                	...
		                	<div class="form-group col-md-12">
                                <div class="row">
                                    ...
                                    <div class="col-lg-3">
                                        <input type="text"  name="giabanle[]" placeholder="Giá bán" class="format-price form-control">
                                    </div>
                                    <!-- <div class="col-lg-3">
...
<script type="text/javascript">
	$(document).on("click",".btn-plus",function(){
			$.ajax ({
				type: "POST",
				url: "ajax/ajax_addmauxe.php",
				dataType: 'html',
				success: function(html) { 
					$("#mauxe-in").append(html);
				}
			});
		});
		$(document).on("click",".btn-del",function(){
			 $(this).parents("#mauxe-in .form-group.col-md-12").remove();
		});
</script>
