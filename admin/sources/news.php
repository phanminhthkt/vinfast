<?php
	...
	function save_item()
	{
		...
		if($id && $act!='save_copy')
		{
			...
			if(isset($_FILES['file-taptin']))
			{
				...
				if($taptin = $func->uploadImage("file-taptin", $config['news'][$type]['file_type'],UPLOAD_NEWS,$file_name."-taptin"))
				{
					$data['taptin'] = $taptin;

					$row = $d->rawQueryOne("select id, taptin from #_news where taptin = ? and com = ? and type = ?  order by stt,id desc limit 0,1",array($idc,$com,$type));
					
					if($row['id']) $func->delete_file(UPLOAD_NEWS.$row['taptin']);
				}
			}
			$data['ngaysua'] = time();
	...
	function delete_item()
	{
		...
		if($id)
		{
			...
			if($row['id'])
			{
				$func->delete_file(UPLOAD_NEWS.$row['taptin']);
