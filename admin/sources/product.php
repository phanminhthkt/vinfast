<?php
	...
	function get_item()
	{
		global $d, $func, $strUrl, $curPage, $item, $gallery, $type, $com, $act,$listmauxe;
		...
		$temps = $d->rawQueryOne("select mauxe from #_product where id = ? and type = ? limit 0,1",array($id,$type));
		$listmauxe = json_decode($temps['mauxe'],true);
	...
	function save_item()
	{
		...
		if($data)
		{
			...
			if(isset($_POST['mauxe']) && ($_POST['mauxe'] != '')){
				$mauxe = $_POST['mauxe'];
				$giabanle = $_POST['giabanle'];
				$khuyenmai = $_POST['khuyenmai'];
				
				$data_mauxe=array();
				foreach($mauxe as $k => $v){
					array_push($data_mauxe,array(
						'mauxe' =>$v,
						'giabanle' => str_replace(",","",$giabanle[$k]),
						'khuyenmai' => str_replace(",","",$khuyenmai[$k]),
					));
				}
				$data['mauxe'] = json_encode($data_mauxe);
			}else{
				$data['mauxe'] = "";
			} 
			...
			$data['phibienso'] = (isset($data['phibienso']) && $data['phibienso']!=='') ? str_replace(",","",$data['phibienso']) : 0;
			$data['phisudungduongbo'] = (isset($data['phisudungduongbo']) && $data['phisudungduongbo']!=='') ? str_replace(",","",$data['phisudungduongbo']) : 0;
			$data['phikiemdinh'] = (isset($data['phikiemdinh']) && $data['phikiemdinh']!=='') ? str_replace(",","",$data['phikiemdinh']) : 0;
			$data['baohiemtnds'] = (isset($data['baohiemtnds']) && $data['baohiemtnds']!=='') ? str_replace(",","",$data['baohiemtnds']) : 0;
		...
		if($id && $act!='save_copy')
		{
			...
			if(isset($_FILES['file_banner']))
			{
				$file_banner = $func->uploadName($_FILES['file_banner']["name"]);
				if($banner = $func->uploadImage("file_banner", $config['product'][$type]['img_type'], UPLOAD_PRODUCT,$file_banner))
				{
					$data['banner'] = $banner;
					$row = $d->rawQueryOne("select id, banner from #_product where id = ? and type = ? limit 0,1",array($id,$type));
					if($row['id']) $func->delete_file(UPLOAD_PRODUCT.$row['banner']);
				}
			}
			$data['ngaysua'] = time();
